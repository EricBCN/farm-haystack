...
from haystack.modeling.data_handler.data_silo import DataSilo, DistillingDataSilo
...
from haystack.modeling.training.base import DistillingTrainer
...
class FARMReader(BaseReader):
    ...
    def _training_procedure(
            ...
            teacher_model: AdaptiveModel = None,
            student_loss_weight: Optional[float] = None
    ):
        """
        ...
        if teacher_model is None:
            ...
            trainer = DistillingTrainer.create_or_load_checkpoint(
                model=model,
                optimizer=optimizer,
                data_silo=data_silo,
                epochs=n_epochs,
    ...
    def distil_from(self,
                    teacher_model: "FARMReader",
                    data_dir: str,
                    train_filename: str,
                    student_loss_weight: float = 0.5,
                    dev_filename: Optional[str] = None,
                    test_filename: Optional[str] = None,
                    use_gpu: Optional[bool] = None,
                    batch_size: int = 10,
        ...
        label_list = ["start_token", "end_token"]
        metric = "squad"
        student_processor = SquadProcessor(
            tokenizer=self.inferencer.processor.tokenizer,
            max_seq_len=max_seq_len,
            label_list=label_list,
            metric=metric,
        ...
        teacher_processor = SquadProcessor(
            tokenizer=teacher_model.inferencer.processor.tokenizer,
            max_seq_len=max_seq_len,
            label_list=label_list,
            metric=metric,
            train_filename=train_filename,
        ...
        # 2. Create a DataSilo that loads several datasets (train/dev/test), provides DataLoaders for them
        # and calculates a few descriptive statistics of our datasets
        data_silo = DistillingDataSilo(processor=student_processor, teacher_processor=teacher_processor,
                                       batch_size=batch_size, distributed=False, max_processes=num_processes)
